...
app.get("/Api/v1/reportingsal", verificaToken, (req, res) => {
  ...
  let body = req.query;
  if (body.fechaini == undefined || body.fechafin == undefined) {
    return res.json({
      status: false,
    ...
    db.executeSql(query, params, "ReportIngSal", (reportingsal, err) => {
      ...
      if (reportingsal.length > 0) {
        ...
        let nombreArchivo = `ReporteIngSal-${new Date().getFullYear()}${
          new Date().getMonth() + 1
        }${new Date().getDate()}${new Date().getSeconds()}${new Date().getMilliseconds()}.xlsx`;
